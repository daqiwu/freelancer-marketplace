    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            result = await db.execute(
                select(Order).where(
                    Order.status == OrderStatus.completed, Order.provider_id.isnot(None)
                )
            )
            ...
            if not completed_orders:
                print(
                    "No completed orders with providers found. Please ensure you have completed orders in the database."
                )
                return

            print(
                f"Found {len(completed_orders)} completed orders eligible for reviews"
            )
            ...
            for order in completed_orders:
                ...
                new_review = Review(
                    ...
                    content=content,
                    created_at=(
                        order.updated_at if order.updated_at else datetime.utcnow()
                    ),
                )
                ...
                reviews_info.append(
                    {
                        ...
                        "content": (
                            ...
                            else content if content else "[No comment]"
                        ),
                ...
                print(
                    f"Created review {len(reviews_info)}: Order #{order.id} - {stars} stars"
                )
            ...
            print("-" * 140)
            print(
                f"{'Review ID':<11} {'Order ID':<10} {'Customer':<20} {'Provider':<20} {'Stars':<7} {'Content Preview':<50}"
            )
            print("-" * 140)

            for review in reviews_info:
                print(
                    f"{review['review_id']:<11} {review['order_id']:<10} {review['customer']:<20} "
            ...
            avg_stars = (
                sum(r["stars"] for r in reviews_info) / len(reviews_info)
                if reviews_info
                else 0
            )
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                f.write("=" * 140 + "\n\n")
                f.write(
                    f"{'Review ID':<11} {'Order ID':<10} {'Customer':<20} {'Provider':<20} {'Stars':<7} {'Full Content':<80}\n"
                )
                f.write("-" * 140 + "\n")
                ...
                for review in reviews_info:
                    ...
                    for r in reviews_info:
                        if r["review_id"] == review["review_id"]:
                            result = await db.execute(
                                select(Review).where(Review.id == review["review_id"])
                            )
                ...
                for stars in range(5, 0, -1):
                    ...
                    percentage = (
                        (count / len(reviews_info) * 100) if reviews_info else 0
                    )
                    f.write(
                        f"  {'⭐' * stars} ({stars} stars): {count} ({percentage:.1f}%)\n"
                    )
            ...
            for stars in range(5, 0, -1):
                ...
                print(
                    f"  {'⭐' * stars} ({stars} stars): {count:3d} ({percentage:5.1f}%) {bar}"
                )
