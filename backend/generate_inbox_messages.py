...
from sqlalchemy import select, Column, Integer, BigInteger, String, Text, ForeignKey, Enum, DECIMAL, TIMESTAMP, DateTime, Boolean
        ...
        try:
            ...
            for order in all_orders:
                ...
                if order.status == OrderStatus.pending_review:
                    ...
                    message = random.choice(CUSTOMER_MESSAGE_TEMPLATES['order_approved'])
                    customer_inbox = CustomerInbox(
                        customer_id=order.customer_id,
                        order_id=order.id,
                        message=f"Order #{order.id}: {message}",
                    ...
                    if providers:
                        selected_providers = random.sample(providers, min(3, len(providers)))
                        for provider in selected_providers:
                            message = random.choice(PROVIDER_MESSAGE_TEMPLATES['new_order_available'])
                            provider_inbox = ProviderInbox(
                                provider_id=provider.id,
                                order_id=order.id,
                                message=f"New Order #{order.id}: {order.title} - {message}",
                                created_at=order_time + timedelta(hours=1, minutes=random.randint(5, 30)),
                                is_read=random.choice([True, False])
                            )
                    ...
                    message = random.choice(CUSTOMER_MESSAGE_TEMPLATES['provider_accepted'])
                    customer_inbox = CustomerInbox(
                        customer_id=order.customer_id,
                        order_id=order.id,
                        message=f"Order #{order.id}: {message}",
                    ...
                    message = random.choice(PROVIDER_MESSAGE_TEMPLATES['order_accepted_confirmation'])
                    provider_inbox = ProviderInbox(
                    ...
                    message = random.choice(CUSTOMER_MESSAGE_TEMPLATES['order_in_progress'])
                    customer_inbox = CustomerInbox(
                        customer_id=order.customer_id,
                        order_id=order.id,
                        message=f"Order #{order.id}: {message}",
                    ...
                    message = random.choice(CUSTOMER_MESSAGE_TEMPLATES['order_completed'])
                    customer_inbox = CustomerInbox(
                        customer_id=order.customer_id,
                        order_id=order.id,
                        message=f"Order #{order.id}: {message}",
                    ...
                    message = random.choice(PROVIDER_MESSAGE_TEMPLATES['payment_received'])
                    provider_inbox = ProviderInbox(
                        provider_id=order.provider_id,
                        order_id=order.id,
                        message=f"Order #{order.id}: {message}",
            ...
            for customer in random.sample(customers, min(10, len(customers))):
                message = random.choice(CUSTOMER_MESSAGE_TEMPLATES['general_notification'])
                customer_inbox = CustomerInbox(
            ...
            for provider in random.sample(providers, min(10, len(providers))):
                message = random.choice(PROVIDER_MESSAGE_TEMPLATES['general_notification'])
                provider_inbox = ProviderInbox(
                    provider_id=provider.id,
                    order_id=None,
                    message=f"System Notification: {message}",
            ...
            await db.commit()
            print("\n" + "="*120)
            print(f"✓ Successfully created {len(customer_messages_info)} customer inbox messages!")
            print(f"✓ Successfully created {len(provider_messages_info)} provider inbox messages!")
            print("="*120 + "\n")
            
            # Print summary
            print("CUSTOMER INBOX MESSAGES (Sample):")
            print("-" * 120)
            print(f"{'ID':<8} {'Customer ID':<13} {'Order ID':<10} {'Message Preview':<70} {'Read':<6}")
            print("-" * 120)
            for msg in customer_messages_info[:10]:
                order_id_str = str(msg['order_id']) if msg['order_id'] else "N/A"
                read_str = "✓" if msg['is_read'] else "✗"
                print(f"{msg['id']:<8} {msg['customer_id']:<13} {order_id_str:<10} {msg['message']:<70} {read_str:<6}")
            ...
            print("-" * 120)
            print(f"{'ID':<8} {'Provider ID':<13} {'Order ID':<10} {'Message Preview':<70} {'Read':<6}")
            print("-" * 120)
            for msg in provider_messages_info[:10]:
                order_id_str = str(msg['order_id']) if msg['order_id'] else "N/A"
                read_str = "✓" if msg['is_read'] else "✗"
                print(f"{msg['id']:<8} {msg['provider_id']:<13} {order_id_str:<10} {msg['message']:<70} {read_str:<6}")
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                for msg in customer_messages_info:
                    ...
                    f.write(f"{msg['id']:<8} {msg['customer_id']:<13} {order_id_str:<10} {read_str:<6}\n")
                ...
                for msg in provider_messages_info:
                    order_id_str = str(msg['order_id']) if msg['order_id'] else "N/A"
                    read_str = "Yes" if msg['is_read'] else "No"
                    f.write(f"{msg['id']:<8} {msg['provider_id']:<13} {order_id_str:<10} {read_str:<6}\n")
            ...
            print("\nMessage Statistics:")
            print(f"Customer Messages: {len(customer_messages_info)} total ({customer_read} read, {customer_unread} unread)")
            print(f"Provider Messages: {len(provider_messages_info)} total ({provider_read} read, {provider_unread} unread)")
