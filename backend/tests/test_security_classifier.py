...
class TestSecurityClassifierService:
    ...
    def test_classify_sast_sql_injection(self, classifier):
        ...
        assert 'parameterized queries' in result['remediation_suggestion'].lower() or \
               'input validation' in result['remediation_suggestion'].lower()
    ...
    def test_classify_dast_missing_security_headers(self, classifier):
        ...
        assert 'api' in json.loads(result['tags']) or 'web' in json.loads(result['tags'])
    ...
    def test_classify_dast_ssl_tls_issue(self, classifier):
        ...
        assert result['issue_type'] == 'DAST'
        assert 'tls' in result['description'].lower() or 'ssl' in result['description'].lower()
