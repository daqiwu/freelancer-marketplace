...
class SecurityClassifierService:
    ...
    def classify_issue(
        self, title: str, description: str, affected_component: str = ""
    ) -> Dict:
        """
    ...
    def _determine_type(
        self, sca_score: float, sast_score: float, dast_score: float
    ) -> Tuple[str, float]:
    ...
    def _generate_remediation(
        self, issue_type: str, severity: str, title: str, component: str
    ) -> str:
        """Generate AI-based remediation suggestion"""
        template = self.REMEDIATION_TEMPLATES.get(issue_type, {}).get(
            severity,
    ...
    def get_statistics(self, classified_issues: List[Dict]) -> Dict:
        ...
        return {
            ...
            "avg_confidence": sum(
                float(i.get("confidence_score", 0)) for i in classified_issues
            )
            / total,
        }
