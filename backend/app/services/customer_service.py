...
async def publish_order(db: AsyncSession, customer_id: int, data):
    ...
    if data.service_start_time:
        service_start_time = dt.fromisoformat(
            data.service_start_time.replace("Z", "+00:00")
        )
    if data.service_end_time:
        service_end_time = dt.fromisoformat(
            data.service_end_time.replace("Z", "+00:00")
        )
    ...
    await send_customer_notification(
        db, customer_id, order.id, "您的订单已发布，等待管理员审核"
    )
    return order
...
async def cancel_order(db: AsyncSession, customer_id: int, order_id: int):
    ...
    await db.refresh(order)
    # 通知客户
    await send_customer_notification(db, customer_id, order.id, f"订单已取消")
    # 如果订单已被接单，通知服务商
    if order.provider_id:
        await send_provider_notification(
            db, order.provider_id, order.id, f"订单 #{order_id} 已被客户取消"
        )
    return order
