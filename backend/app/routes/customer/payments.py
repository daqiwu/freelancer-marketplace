...
from app.models.models import (  # 导入所需模型 # Import necessary models
    CustomerProfile,
    Order,
    OrderStatus,
    Payment,
...
async def recharge_balance(
    ...
    if data.amount <= 0:
        raise HTTPException(
            status_code=400,
            detail="充值金额必须大于0  // Recharge amount must be greater than 0",
        )
    ...
    if not profile:
        raise HTTPException(
            status_code=404, detail="未找到客户资料  // Customer profile not found"
        )
...
async def pay_order(
    ...
    result = await db.execute(
        select(Order).where(
            Order.id == data.order_id, Order.customer_id == current_user_id
        )
    )
    ...
    await send_customer_notification(
        db, current_user_id, order.id, f"订单 #{order.id} 支付成功"
    )
    if order.provider_id:
        await send_provider_notification(
            db, order.provider_id, order.id, f"订单 #{order.id} 已收到付款"
        )

    return PayOrderResponse(
        order_id=order.id, transaction_id=transaction_id, message="支付成功"
    )
