...
@review_router.post("/", response_model=CreateReviewResponse, status_code=status.HTTP_201_CREATED)
...
async def get_my_provider_rating(current_user_id: int = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """获取当前 Provider 的评分（需要认证）"""
    from sqlalchemy import func
    
    result = await db.execute(
        select(func.avg(Review.stars), func.count(Review.id))
        .where(Review.provider_id == current_user_id)
    )
...
async def get_my_provider_reviews(current_user_id: int = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    """获取当前 Provider 的所有评价（需要认证）"""
    result = await db.execute(select(Review).where(Review.provider_id == current_user_id))
...
@review_router.get("/provider/{provider_id}/rating", response_model=ProviderRatingResponse)
