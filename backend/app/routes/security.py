...
from app.models.models import (
    SecurityIssue, SecurityIssueType, SecuritySeverity, 
    SecurityIssueStatus, User
)
...
@security_router.post("/issues", response_model=SecurityIssueResponse, status_code=status.HTTP_201_CREATED)
...
async def get_security_issues(
    ...
    try:
        ...
        if status_filter:
            conditions.append(SecurityIssue.status == SecurityIssueStatus[status_filter])
        
        if conditions:
            query = query.where(and_(*conditions))
        
        # Apply pagination
        query = query.offset(skip).limit(limit).order_by(SecurityIssue.created_at.desc())
        ...
        response = []
        for issue in issues:
            response.append(SecurityIssueResponse(
                **{
                    **issue.__dict__,
