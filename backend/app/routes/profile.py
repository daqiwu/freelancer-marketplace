...
async def get_my_profile(
    ...
    if not profile:
        raise HTTPException(
            status_code=404, detail="用户信息不存在  // User profile not found"
        )
    return profile
...
async def update_customer_profile_api(
    ...
    from app.models.models import CustomerProfile, LocationEnum, User
    ...
    if not profile:
        raise HTTPException(
            status_code=404, detail="客户资料不存在  // Customer profile not found"
        )
    ...
    try:
        ...
        return {
            "msg": "客户资料更新成功  // Customer profile updated",
...
async def update_provider_profile_api(
    ...
    if not profile:
        raise HTTPException(
            status_code=404, detail="服务商资料不存在  // Provider profile not found"
        )
    ...
    try:
        ...
        return {
            "msg": "服务商资料更新成功  // Provider profile updated",
            "profile_id": profile.id,
        }
    except Exception as e:
        await db.rollback()
