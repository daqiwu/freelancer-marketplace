...
@admin_orders_router.get(
    ...
    responses={
        401: {
            ...
            "content": {
                "application/json": {"example": {"error": "Authentication required."}}
            },
        },
        422: {
...
async def list_all_orders_route(
    ...
    try:
        orders = await list_all_orders(
            db, status=status, page=page, limit=limit, sort_by=sort_by, order=order
        )
        items = [
            AdminOrderItem(
                id=o.id,
        ...
        raise HTTPException(
            ...
            detail={"error": "Validation failed.", "details": str(ve)},
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "Error Fetching orders", "details": str(e)},
        )
...
async def get_pending_review_orders_route(
    db: AsyncSession = Depends(get_db), current_user_id: int = Depends(get_current_user)
):
    """获取所有待审核订单"""
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "Error fetching pending review orders", "details": str(e)},
        )
...
@admin_orders_router.post(
    "/orders/{order_id}/approve", response_model=ApproveOrderResponse
)
async def approve_order_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "Error approving order", "details": str(e)},
        )
...
async def update_order_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"error": "Error updating order", "details": str(e)}
        )
...
async def delete_order_route(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail={"error": "Error deleting order", "details": str(e)}
        )
