...
async def generate_payments():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            if not all_orders:
                print("Error: No orders found in database. Please generate orders first.")
                return
            ...
            for order in all_orders:
                ...
                if should_create_payment and random.random() < 0.05:  # 5% chance of failure
                ...
                transaction_id = generate_transaction_id(order.id, order.customer_id, timestamp)
                ...
                payments_info.append({
                    ...
                    "order_id": order.id,
                    "customer_id": order.customer_id,
                    "provider_id": order.provider_id if order.provider_id else "None",
                ...
                print(f"Created payment {len(payments_info)}: Order #{order.id} - ${order.price} - {payment_status_enum.value}")
            ...
            print("-" * 130)
            print(f"{'Payment ID':<12} {'Order ID':<10} {'Customer ID':<13} {'Provider ID':<13} {'Amount':<10} {'Status':<12} {'Transaction ID':<25}")
            print("-" * 130)
            
            for payment in payments_info:
                print(f"{payment['payment_id']:<12} {payment['order_id']:<10} {payment['customer_id']:<13} "
            ...
            with open(output_file, "w", encoding="utf-8") as f:
                ...
                f.write("=" * 130 + "\n\n")
                f.write(f"{'Payment ID':<12} {'Order ID':<10} {'Customer ID':<13} {'Provider ID':<13} {'Amount':<10} {'Status':<12} {'Transaction ID':<25}\n")
                f.write("-" * 130 + "\n")
                
                for payment in payments_info:
                    f.write(f"{payment['payment_id']:<12} {payment['order_id']:<10} {payment['customer_id']:<13} "
                           f"{str(payment['provider_id']):<13} ${payment['amount']:<9.2f} {payment['status']:<12} {payment['transaction_id']:<25}\n")
