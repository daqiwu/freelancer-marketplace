...
from app.models.models import User, Role, CustomerProfile, ProviderProfile, LocationEnum
...
async def generate_users():
    ...
    async with AsyncSessionLocal() as db:
        try:
            ...
            print("-" * 80)
            print(f"{'No.':<5} {'Username':<25} {'Password':<20} {'Email':<35} {'Role':<10}")
            print("-" * 80)
            
            for idx, cred in enumerate(user_credentials, 1):
                print(f"{idx:<5} {cred['username']:<25} {cred['password']:<20} {cred['email']:<35} {cred['role']:<10}")
            ...
            with open("generated_users.txt", "w", encoding="utf-8") as f:
                ...
                f.write("=" * 80 + "\n\n")
                f.write(f"{'No.':<5} {'Username':<25} {'Password':<20} {'Email':<35} {'Role':<10}\n")
                f.write("-" * 80 + "\n")
                
                for idx, cred in enumerate(user_credentials, 1):
                    f.write(f"{idx:<5} {cred['username']:<25} {cred['password']:<20} {cred['email']:<35} {cred['role']:<10}\n")
